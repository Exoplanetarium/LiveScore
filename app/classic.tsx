...
export default function AnalyzeScreen() {
  ...
  // Key signature state (MusicXML fifths: negative = flats, positive = sharps)
  const [keySignature, setKeySignature] = useState<number>(0);
  const KEY_SIGNATURES: { fifths: number; label: string }[] = [
    { fifths: -7, label: "Cb" },
    { fifths: -6, label: "Gb" },
    { fifths: -5, label: "Db" },
    { fifths: -4, label: "Ab" },
  ...
  // Export MusicXML file via share sheet
  const exportMusicXML = async () => {
    ...
    try {
      ...
      const fileUri = FileSystem.cacheDirectory + "score.musicxml";
      ...
      Alert.alert("Export Failed", e.message || "Could not export MusicXML.");
    }
  };

  // Export MIDI file via share sheet
    ...
    try {
      ...
      for (const n of analysisResults.details.notes || []) {
        ...
        track.addNote({
          ...
          velocity: n.confidence ?? 0.8,
        });
      }
      // Add chord notes
      for (const c of analysisResults.details.chords || []) {
      ...
      const fileUri = FileSystem.cacheDirectory + "score.mid";
      ...
      Alert.alert("Export Failed", e.message || "Could not export MIDI.");
    }
  };
          ...
          <View style={styles.resultsSummary}>
            ...
            <View style={styles.exportRow}>
              ...
              >
                ...
                <Text style={styles.exportButtonText}>Export MusicXML</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.exportButton}
                onPress={exportMIDI}
        ...
        {analysisResults &&
          ...
          analysisResults.notes.length > 0 && (
            <View style={styles.sheetMusicSection}>
              ...
              <View style={styles.keySignatureSection}>
                <Text style={styles.keySignatureLabel}>Key Signature:</Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.keySignatureRow}
                >
                  {KEY_SIGNATURES.map((ks) => (
                    <TouchableOpacity
                      key={ks.fifths}
                      style={[
                        styles.keySignatureButton,
                        keySignature === ks.fifths &&
                          styles.keySignatureButtonActive,
                      ]}
                      onPress={() => setKeySignature(ks.fifths)}
                    >
                      <Text
                        style={[
                          styles.keySignatureText,
                          keySignature === ks.fifths &&
                            styles.keySignatureTextActive,
                        ]}
                      >
                        {ks.label}
                      </Text>
